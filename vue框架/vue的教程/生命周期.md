...
* updated()不会保证所有的子组件都被一起重绘，如果想要在整个视图都重绘完毕的话，可以在update中使用vm.$nextTick()
* this.$nextTick()将回调延迟到下次 DOM 更新循环之后执行。在修改数据之后立即使用它，然后等待 DOM 更新。
* 目前的理解是，这是个异步的任务。但是并不保证在axios的请求之后调用。初始化scroll需要放在.then中, 要是封装组件的话需要watch一下data， 然后使用this.nextTick
* Vue 数据发生变化（this.data = res.data）到页面重新渲染是一个异步的过程，我们的初始化时机是要在 DOM 重新渲染后，所以这里用到了 this.$nextTick，当然替换成 setTimeout(fn, 20) 也是可以的。(Vue的DOM渲染在哪个阶段？ 如何确保nextTick在DOM更新后？)

## 有关scroll的初始化的过程

* 可以在created中请求axios， 并在axios的.then中初始化scroll。尽管created中拿不到DOM。但是因为axios的结果是个异步的过程。等到.then方法执行。DOM早就被渲染好了。
* 如果内容存在图片的时候， 可能会出现DOM元素已经渲染， 但是图片还未下载的情况。这个图片的异步不是axios请求的异步， 而是浏览器解析页面时的异步。
* 是不是如果有图片的话，给img预留出空间就好了。试一试