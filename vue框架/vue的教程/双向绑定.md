 ...
 * @LastEditTime: 2022-07-12 11:20:59
...
* input事件使用$event.target.value来获取当前表单的最新值
...
* 一个组件上的v-model会默认利用名为 value 的 prop 和名为 input 的事件，但是像单选框，复选框类型的输入控件会不同，所以引用model模块来规范
