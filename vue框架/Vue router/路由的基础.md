...
* 默认情况下，被选中的router-link都会被添加一个类名，也可以去router的实例里面添加一个linkActiveClass的属性，用来修改默认添加的类名。也可以只在当前的router-link上添加一个active-class属性来指定默认添加的类名。

## $router属性

* vue-router给每一个组件都添加了$router的属性
* 通过this.$router可以访问到路由对象实例
