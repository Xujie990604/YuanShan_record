...
* cookie是不可跨域的
* 浏览器第一次发出请求，服务器会将 cookie 放入到响应请求中，在浏览器第二次发请求的时候，会把 cookie 带过去，于是服务端就会辨别用户身份
* cookie是保留在浏览器端的
* cookie不设置过期时间的话，生命随着浏览器的关闭而结束。
...
* 生命周期和当前浏览器会话有关系，当会话结束后数据会自动清除，和未设置过期时间的Cookie类似。
* 适用于敏感账号登录
