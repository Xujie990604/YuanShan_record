...
javascript:;在a的href；里面的作用是防止页面跳转(执行空代码)。#的作用差不多不过是返回页面顶部

## 隐含的css转化

* 制作横向导航栏的时候，可以ul设置position absolute ul会被内部转化为inline-block(触发bfc能够包裹float元素)
* 行内元素设置完浮动之后就可以手动设置宽高了
